...
class RacetrackEnv(Env):
    ...
    def step(self, action: int):
        ...
        if (action < 0 or action > 8):
            raise ValueError(
                "action must be an integer in the range [0-8] corresponding to one of the legal actions.\n" +
        ...
        if (new_position[0] < 0 or new_position[1] < 0 or new_position[0] >= self.track.shape[0] or new_position[1] >=
                self.track.shape[1]):
    ...
    def render(self, render_mode='human'):
        ...
        if (not self.velocity == (0, 0)):
            ax.arrow(self.position[1], self.track.shape[0] - 1 - self.position[0], self.velocity[1], -self.velocity[0],
                     path_effects=[pe.Stroke(linewidth=1, foreground='black')], color="yellow", width=0.1,
